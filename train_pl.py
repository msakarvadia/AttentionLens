...
parser.add_argument(
    "--max_ckpt_num", default=1, type=int, help="maximum number of ckpts to save"
)
parser.add_argument("--batch_size", default=1, type=int)
parser.add_argument("--num_nodes", default=1, type=int)
parser.add_argument(
    "--model_name", default="gpt2-small", type=str, choices=["gpt2-small", "gpt2-large"]
)
parser.add_argument(
    "--mixed_precision",
    default=True,
...
parser.add_argument(
    ...
    default="checkpoint",
    type=str,
    help="directory to store checkpoint files in; if dir already has ckpt files and user didn't "
    "specity a differnt ckpt dir to reload from (via relaod_checkpoint), we will reload "
    "latest ckptfile in this dir",
)
parser.add_argument(
...
class LightningLens(pl.LightningModule):
    def __init__(self):
        ...
        lens_param = {
            "unembed": self.base_model.W_U,
            "bias": self.base_model.b_U,
            "n_head": self.base_model.cfg.n_heads,
            "d_model": self.base_model.cfg.d_model,
            "d_vocab": self.base_model.cfg.d_vocab,
            "lense_class": LenseA,
        }
    ...
    def setup(self, stage):
        self.model = get_model(
            model_name=args.model_name, device=self.trainer.strategy.root_device
        )
        return
...
checkpoint_callback = (
    train_loss_checkpoint if args.checkpoint_mode == "loss" else step_checkpoint
)
